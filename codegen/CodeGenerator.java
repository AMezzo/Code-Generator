...
public Object visit(IterationTree node) throws Exception {
    ...
    String continueLabel = createLabel("continue");
    String internalLoopStartLabel = createLabel("internalLoopStart");
    ...
    int iterCountOffset = currentFrameSize();
    storeCode(new VarOpCode(Codes.ByteCodes.LIT, 0, "iterCount"));
    changeFrameSize(1);
    storeCode(new VarOpCode(Codes.ByteCodes.STORE, iterCountOffset, "iterCount"));

    storeCode(new LabelCode(Codes.ByteCodes.LABEL, internalLoopStartLabel));

    storeCode(new VarOpCode(Codes.ByteCodes.LOAD, iterCountOffset, "iterCount"));
    storeCode(new NumOpCode(Codes.ByteCodes.LIT, 0));
    ...
    storeCode(new VarOpCode(Codes.ByteCodes.LOAD, iterCountOffset, "iterCount"));
    storeCode(new NumOpCode(Codes.ByteCodes.LIT, 1));
    storeCode(new LabelCode(Codes.ByteCodes.BOP, "-"));
    storeCode(new VarOpCode(Codes.ByteCodes.STORE, iterCountOffset, "iterCount"));

    storeCode(new LabelCode(Codes.ByteCodes.GOTO, internalLoopStartLabel)); 
    storeCode(new LabelCode(Codes.ByteCodes.LABEL, continueLabel));
